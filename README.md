...
### ジョブマッチアプリのサーバ開発
【プロジェクト概要】
国外のジョブマッチアプリケーションのバックエンドを新規開発しました。
メインエンジニアとしてマネージャの方と二人でサーバ開発を行いました。

フロントのスマホアプリ実装は海外の企業の実装でしたので、細かい調整はテキストベースで英語で行いました。
...
### 賃貸契約アプリのサーバ開発
【業務内容】
代表と二人でお客様の要件定義から行い、社内のフロントエンドの方と、マネージャの方と開発を行いました。
技術自体はジョブマッチアプリのものと同じですが、toCとtoBとadminと3分類の実装を短期間で行った事が大変でした。
AWSインフラ構築について社内の方に手伝って頂きながらCloudFormationで作成しました。
...
### 他社家計簿アプリのスクレイピングのエラー修正
【業務内容】
家計簿アプリ上でのスクレイパーについて、銀行等のui仕様変更などに追随してエラー修正を行う業務を行いました。
ひと月あたり、3人チームで50個のバグ修正を行いました。
...
### 広告配信システムの筐体上で動作するelectronアプリケーション、インフラ構築
【業務内容】
広告配信アプリ実装を行いました。
広告配信筐体にraspberry piを画面出力し、動画やpdfを表示するアプリケーションを作成しました。
当初は既存の動画管理サーバを用いたものとして開発しておりましたが、お客様の予算の都合上そのサーバは使えないとの事で社内で動画サーバを作成し直したため、急遽大きい仕様変更があったのが大変でした。
